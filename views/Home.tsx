...
import { DEV_NAME, DEV_TITLE, PROJECTS, SKILL_TREE, QUEST_LOG } from '../constants';
...
const Home: React.FC<HomeProps> = ({ onChangeView }) => {
  ...
  const featuredSkills = SKILL_TREE.slice(0, 4);
  const latestQuest = QUEST_LOG[0];

  // Increased pt to 32/36 to clear the taller header area
  return (
    <section className="min-h-screen pt-32 md:pt-36 pb-20 px-4 max-w-7xl mx-auto relative">
      
      {/* Hero Section - The Avatar */}
      <div className={`flex flex-col md:flex-row items-center gap-8 md:gap-16 mb-16 md:mb-20 transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        {/* Avatar Frame */}
        <div className="relative group shrink-0">
          ...
          <GoldenFrame variant="heavy" className="p-2 md:p-3 rounded-full md:rounded-lg rotate-0 md:rotate-3 transition-transform duration-500 hover:rotate-0">
            <div className="w-32 h-32 md:w-56 md:h-56 overflow-hidden border-2 border-charcoal bg-black">
              <img 
        ...
        <div className="text-center md:text-left space-y-4 md:space-y-6 max-w-2xl">
          <div className="flex items-center justify-center md:justify-start gap-2 text-gold/60 font-cinzel text-xs md:text-sm tracking-widest uppercase">
            <Crown size={14} className="md:w-4 md:h-4" /> Lvl. 99 {DEV_TITLE}
          </div>
          
          <h1 className="font-cinzel text-3xl sm:text-5xl md:text-7xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white via-neutral-200 to-neutral-600 drop-shadow-xl leading-tight">
            {DEV_NAME}
          </h1>
          
          <p className="font-sans text-neutral-400 text-sm md:text-lg leading-relaxed border-l-0 md:border-l-2 border-gold/30 pl-0 md:pl-6 ml-0 md:ml-0 px-4 md:px-0">
           ...
           <GoldenFrame variant="heavy" className="h-full bg-charcoal/40">
             <div className="flex flex-col h-full">
                <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                  <h3 className="font-cinzel text-lg md:text-xl text-gold flex items-center gap-2">
                    <Gem size={18} className="text-gold-bright" /> The Forged Artifacts
                  </h3>
                  <span className="font-mono text-[10px] md:text-xs text-neutral-500 uppercase tracking-wider">Featured Project</span>
                </div>

                <div className="flex flex-col md:flex-row gap-6 flex-1">
                   {/* Thumbnail */}
                   <div className="w-full md:w-1/2 h-40 md:h-auto overflow-hidden border border-neutral-700 relative group cursor-pointer" onClick={() => onChangeView(ViewState.PORTFOLIO)}>
                   ...
                   <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="font-cinzel text-xl md:text-2xl text-neutral-100 mb-2">{featuredProject.title}</h4>
                        <div className="flex flex-wrap gap-2 mb-4">
                          {featuredProject.techStack.map(tech => (
                             <span key={tech} className="text-[10px] md:text-xs font-mono text-neutral-400 bg-black/40 px-2 py-1 border border-neutral-800">{tech}</span>
                          ))}
                        </div>
          ...
          <GoldenFrame variant="light" className="h-full bg-charcoal/40">
             <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                ...
                <div className="flex items-center gap-2">
                   ...
                   <span className="font-mono text-[10px] md:text-xs text-neutral-500">Live Feed</span>
                </div>
             </div>
             ...
             <div className="relative pl-6 border-l border-neutral-800 ml-2">
                ...
                <h4 className="font-cinzel text-base md:text-lg text-neutral-200 mb-1">{latestQuest.role}</h4>
